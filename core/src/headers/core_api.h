...
CORE_API DocumentId mc_open_document(StyleData style, Cut * data, unsigned size);//CHECK
CORE_API unsigned mc_get_storage_buffer_size(DocumentId docId);//check
CORE_API Cut * mc_close_document(DocumentId docId);//check
...
CORE_API RenderingBuffersSizes mc_get_rendering_buffers_sizes(DocumentId docId);//check
...
CORE_API ObjectId * mc_get_all_objects(DocumentId docId);//check
CORE_API Status mc_highlight_object(DocumentId docId, ObjectId objId);

/*
CORE_API int testApi(cTest * arr, unsigned size);
CORE_API cTest * test2Api();
*/
